...
import com.word2vect.domain.WordNeuron;
import com.word2vect.util.Haffman;
import com.word2vect.util.MapCount;
    ...
    private void skipGram(int index, List<WordNeuron> sentence, int b) {
        ...
        for (a = b; a < window * 2 + 1 - b; a++) {
            ...
            for (int i = 0; i < neurons.size(); i++) {
                ...
                // com.Learn weights hidden -> output
                for (c = 0; c < layerSize; c++) {
                    out.syn1[c] += g * we.syn0[c];
                }
            }

            // com.Learn weights input -> hidden
            for (int j = 0; j < layerSize; j++) {
                we.syn0[j] += neu1e[j];
            }
        }
